  ...
  return (
    <section className="relative overflow-hidden bg-background-light dark:bg-background-dark">
      <div className="mx-auto w-full max-w-7xl px-4 md:px-8">
        <div className="grid grid-cols-1 gap-8 py-6 lg:grid-cols-2 lg:gap-16 lg:py-10">
          {items.map((item, index) => (
            <article
              key={`${item.kicker}-${index}`}
              id={item.kicker === "THE ANSWER" ? "the-answer" : undefined}
              className="flex min-h-[400px] flex-col justify-center rounded-2xl border border-slate-200 bg-white p-8 shadow-xl dark:border-slate-700 dark:bg-slate-900 lg:min-h-[480px] lg:p-16"
            >
              ...
              {index === 0 ? (
                <div className="mt-8 rounded-xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-800">
